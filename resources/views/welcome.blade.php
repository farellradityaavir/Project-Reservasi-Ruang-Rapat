...
<!-- ANIMASI -->
<style>
@keyframes navbarFadeDown {0% { opacity: 0; transform: translateY(-25px); }100% { opacity: 1; transform: translateY(0); }}
...
<nav id="navbar" class="fixed w-full z-50 bg-white/80 backdrop-blur-xl shadow-sm border-b border-gray-200 animate-navbarFadeDown transition-all duration-500">
    <div class="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        <div class="flex items-center space-x-4">
            <img src="{{ asset('images/logo.png') }}" class="h-11 w-11 rounded-xl shadow-lg animate-logoFloat transition-transform duration-500 hover:scale-110 hover:drop-shadow-lg">
...
<section class="pt-40 pb-36 bg-cover bg-center relative" style="background-image:url('https://images.unsplash.com/photo-1504384308090-c894fdcc538d');">
    ...
    <div class="relative max-w-5xl mx-auto px-6 text-center">
        ...
        <a href="{{ route('login') }}" class="bg-white/95 text-red-700 px-10 py-4 rounded-2xl text-xl font-bold shadow-xl hover:bg-red-700 hover:text-white transition animate-fadeSoft">Mulai Booking</a>
    </div>
</section>
...
<section id="jadwal" class="py-24 animate-fadeSoft">
    <div class="max-w-7xl mx-auto px-6 text-center">
        <h3 class="text-4xl font-bold text-gray-900 mb-6">Jadwal Ketersediaan Ruangan</h3>
        <p class="text-gray-600 mb-10 max-w-xl mx-auto">Pilih waktu yang tersedia untuk melakukan pemesanan ruang rapat secara cepat dan efisien.</p>

        <div class="grid md:grid-cols-3 gap-8">
            @foreach($tempatRapat as $tempat)
            <div class="bg-white shadow-lg p-6 rounded-3xl text-gray-700 border border-gray-200">
                <h4 class="text-xl font-semibold mb-2">{{ $tempat->nama }}</h4>
                <p class="mb-1"><strong>Alamat:</strong> {{ $tempat->alamat }}</p>
                <p class="mb-1"><strong>Fasilitas:</strong> {{ $tempat->fasilitas }}</p>
                <p class="mb-2"><strong>Reservasi Hari Ini:</strong> {{ $tempat->reservasis->count() }}</p>
                @if($tempat->reservasis->count() > 0)
                    <p class="text-red-600 font-semibold">Beberapa slot sudah terbooking</p>
                @else
                    <p class="text-green-600 font-semibold">Semua slot tersedia</p>
                @endif
            </div>
            @endforeach
        </div>
...
window.addEventListener('scroll', () => {
    if(window.scrollY > 50) navbar.classList.add('nav-small');
    else navbar.classList.remove('nav-small');
